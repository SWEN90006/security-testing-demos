...
**For Mac M1-5 users**, use the following command instead: 

```bash
docker build --platform=linux/amd64 . -t swen90006 --no-cache
```
...
afl-fuzz -m none -d -i in-afl -o out-afl -- ./good_bad_fuzz @@
```

In this example, we use four options for `afl-fuzz`, which is the main fuzzing command of AFL. The first option (`-m none`) is optional. It removes the memory limit, which is set to 50 MB by default. The second option (`-d`) enables AFL mode in which only havoc mutations are used and the deterministic mode is disabled. There are pros and cons to this option, but it normally produces good results. The third option (`-i in-afl`) and the fourth option (`-o out-afl`) specify the folders from which AFL takes the initial seed corpus and where it stores the results, such as crashing inputs, the updated corpus, and statistics for analysis.
...
cd $WORKDIR
mkdir in-png-afl
cp libpng-afl/*.png in-png-afl
afl-fuzz -m none -d -i in-png-afl -o out-png-afl -- libpng-afl/pngimage @@
```
